% Check if a value is a multiple of X and plot it.
\newcommand\ifismultiple[4]{%
    \pgfmathparse{mod(#1,#2)==0}
    \ifnum \pgfmathresult=1
    #3%
    \else
    #4%
    \fi
}

\begin{figure}
    \centering
\begin{tikzpicture}[scale=0.6, style={node contents={}}]
  \foreach \y [count=\n] in {
      {74,25,39,20,3,3,3,3,3},
      {25,53,31,17,7,7,2,3,2},
      {39,31,37,24,3,3,3,3,3},
      {20,17,24,37,2,2,6,5,5},
      {3,7,3,2,12,1,0,0,0},
      {3,7,3,2,1,36,0,0,0},
      {3,2,3,6,0,0,45,1,1},
      {3,3,3,5,0,0,1,23,1},
      {3,2,3,5,0,0,1,1,0},
    } {
      % column labels

      \ifismultiple{\n}{2}{\node[minimum size=6mm] at (\n, 0) {\n};}{}
        % {
        % \pgfmathtruncatemacro{\itest}{ifthenelse(\n==5||\n==7,1,0)}
        %     \ifnum\itest=1
        %          \node[minimum size=6mm] at (\n, 0) {\n};
        %     \fi
        % }
        
      % \ifnum \n < 10
      %   \node[minimum size=6mm] at (\n, 0) {\n};
      % \fi
      % heatmap tiles
      \foreach \x [count=\m] in \y {
        % With text in box
        % \node[fill=yellow!\x!purple, minimum size=6mm, text=white, font=\tiny] at (\m,-\n) {\x};
        % Without text in box
        \node[fill=yellow!\x!purple, minimum size=6mm, text=white, font=\tiny] at (\m,-\n) {};
      }
    }

  % row labels
  \foreach \a [count=\i] in {a,b,c,d,e,f,g,x,i} {
    \node[minimum size=6mm] at (0,-\i) {\a};
  }
\end{tikzpicture}
    % \includegraphics{}
    \caption{Heatmap without text in cell.}
    \label{fig:heatmap}
\end{figure}